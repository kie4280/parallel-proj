...
inline bool is_dark(int sq) {
  bool dark =
      ...
      ||
      ((sq & 8) &&
       !(sq & 1));  // eg (a7,b7,c7...h7) && (a7|c7|e7|g7) even rank + even file
  return dark;
}
...
class ChessRules : public ChessPosition {
 public:
  ...
  void Init()  // TODO == ChessRules::Init() should call ChessPosition::Init()
               // right ????!!!! Thoughts: Maybe - but can't do this casually.
               // For example we would need to change the code that converts
               // ChessPosition to ChessRules below, both the copy constructor
               // and assignment operator use ChessRules::Init() at a time when
               // it would be disastrous to set the initial position (because we
               // have carefully copied a position into the ChessRules object)
  {
  ...
  bool Forsyth(const char *txt) {
    ...
    if (okay) Init();  // clear stuff for repitition, 50 move rule
    return okay;
  }
